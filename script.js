...
let currentTopicBaseId = null;
        ...
        function showMenuPage(pageId, fromPopState = false) {
            ['page-topics', 'page-path', 'page-repetition'].forEach(id => {
        ...
        function navigateToMenuTab(tab, fromPopState = false) {
            if (tab === activeMenuTab && !fromPopState) {
                const currentPage = tab === 'topics' ? 'page-topics' : 'page-repetition';
                if (!document.getElementById(currentPage).classList.contains('hidden')) return;
            }

            if (!fromPopState) {
                const page = tab === 'topics' ? 'topics' : 'repetition';
            ...
            showMenuPage(tab === 'topics' ? 'page-topics' : 'page-repetition', fromPopState);
            window.scrollTo(0, 0);
        }
